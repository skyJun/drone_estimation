...
def plot():        
    ...
    def update_plot(frame):               
        if data_queue.empty():            
            ...
            time_label.append(len(drone_trajectory))           
            
            ax.plot(time_label, drone_trajectory , marker = 'x', markersize = 10)        
            ax.set_xlabel('time')
            ax.set_ylabel('distance')

            ax.set_title('distance between two drone')
...
def run_sequence(scf):
    ...
    while time.time() < end_time:  
        ...
        #d = [data.d1,data.d2,data.d4]
        #print(d)            
        print('distance between drone : ', data.d4)
        data_queue.put([data.d4])
        time.sleep(0.1)
...
if __name__ == '__main__':
    ...
    with SyncCrazyflie(uri_target, cf=Crazyflie(rw_cache='./cache')) as scf_t, SyncCrazyflie(uri_anchor, cf=Crazyflie(rw_cache='./cache')) as scf_a :
        ...
        t2 = threading.Thread(target=run_sequence_2, args=(scf_a, ))
        t2.start()
        threads.append(t2)
